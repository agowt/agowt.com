  ...
  func storeFile(withProvider provider: NSItemProvider, _ semaphore: DispatchSemaphore) {
    ...
    if provider.hasItemConformingToTypeIdentifier("com.adobe.pdf") {
      provider.loadItem(forTypeIdentifier: "com.adobe.pdf", options: nil) { (data, error) in
        ...
        guard let url = data as? URL else {
          self.exit(withError: "Could not load PDF URL")
          return
        }
        ...
        if extractedText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
          extractedText = "[PDF contained no extractable text — scanned document?]"
        }

        self.sharedItems.append([DATA_KEY: extractedText.trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    // ── Image (WhatsApp screenshot, photo): OCR via Vision ───────────────────
    let imageTypes = [
    ...
    if imageTypes.contains(where: { provider.hasItemConformingToTypeIdentifier($0) }) {
      let typeId = imageTypes.first(where: { provider.hasItemConformingToTypeIdentifier($0) }) ?? kUTTypeImage as String
      provider.loadItem(forTypeIdentifier: typeId, options: nil) { (data, error) in
        ...
        } else if let imageData = data as? Data, let uiImage = UIImage(data: imageData) {
          cgImage = uiImage.cgImage
        }

        guard let cg = cgImage else {
          self.sharedItems.append([DATA_KEY: "[Image: could not load for OCR]",
                                    MIME_TYPE_KEY: "text/plain"])
          semaphore.signal()
          return
        }
        ...
        let request = VNRecognizeTextRequest { req, err in
          guard err == nil else {
            self.sharedItems.append([DATA_KEY: "[Image: OCR failed]",
                                      MIME_TYPE_KEY: "text/plain"])
            semaphore.signal()
            return
          }
          let recognised = req.results?
            .compactMap({ $0 as? VNRecognizedTextObservation })
            .compactMap({ $0.topCandidates(1).first?.string })
            .joined(separator: "\n") ?? ""

          let text = recognised.trimmingCharacters(in: .whitespacesAndNewlines)
          self.sharedItems.append([DATA_KEY: text.isEmpty ? "[Image: no text detected]" : text,
                                    MIME_TYPE_KEY: "text/plain"])
          semaphore.signal()
        }
    ...
    // ── All other binary files: store file path (original behaviour) ──────────
