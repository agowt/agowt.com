...
//
// Uses Swift-native closure types instead of RCTPromiseResolveBlock /
// RCTPromiseRejectBlock to avoid the bridging header dependency.
// The Objective-C bridge (YoustoCalendar.m) handles type translation.
...
// Swift-native equivalents of RCTPromiseResolveBlock / RCTPromiseRejectBlock.
// ObjC bridge translates these automatically — no bridging header needed.
...
private class CalendarEditDelegate: NSObject, EKEventEditViewDelegate {
  private let resolve: ResolveBlock
  private let reject:  RejectBlock

  init(resolve: @escaping ResolveBlock, reject: @escaping RejectBlock) {
